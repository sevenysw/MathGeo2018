function [ image_resto ,p_resto] = write_patches_tv_sor( patches, patch_size,r_sub,c_sub )
...
tv=patches(end-1,:);%figure;plot(tv)
judge=ones(size(tv));
...
%              +patch_size(2)-2)+d;   
         ...
         p_resto(r_sub(i):r_sub(i)+patch_size(1)-2,c_sub(j):c_sub(j)...
             +patch_size(2)-2)=p_resto(r_sub(i):r_sub(i)+patch_size(1)-2,c_sub(j):c_sub(j)...
