...
{ 
    double *x, *b, *s, *v;
	double vtmp, v_, sm, rnd;
    mwSize sz[3], stepsize[3], n[3], ndims;
    mwIndex i, j, k, l, m,n2, blocknum;
    ...
    v = mxGetPr(mxCreateDoubleMatrix(sz[0]*sz[1]*sz[2], 1, mxREAL));
	...
	for (k=0; k<=n[2]-sz[2]; k+=stepsize[2]) {
      for (j=0; j<=n[1]-sz[1]; j+=stepsize[1]) {
        for (i=0; i<=n[0]-sz[0]; i+=stepsize[0]) {
          ...
          for (m=0; m<sz[2]; m++) {
            for (l=0; l<sz[1]; l++) {
              memcpy(v, x+(k+m)*n[0]*n[1]+(j+l)*n[0]+i, sz[0]*sizeof(double));
            }
          }
		  sm = 0;
		  for (m=0; m<sz[2]; m++) {
		  ...
		  v_ = 0;
		  sm = sm/ (sz[0]*sz[1]*sz[2]);
		  for (m=0; m<sz[2]; m++) {
            for (l=0; l<sz[1]; l++) {
              for (n2=0; n2<sz[0]; n2++) {
				  v_ = v_ + (v[m][l][n2]-sm)*(v[m][l][n2]-sm);
			  }
            }
          }
		  v_ = v_/(sz[0]*sz[1]*sz[2]);
		  if (v_>vtmp)
			  vtmp = v_;
