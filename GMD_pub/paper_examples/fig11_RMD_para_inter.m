% RMD2D interpolation test on hyperbolic events
...
mask = f0 * 0;
mask(:,1:4:end) = 1;
f = f0 + 0.2*randn(size(f0));
f = mask.*f;
% parameters:
alpha = 0.002;       % bandwidth constraint
tau = 0.00;         % Lagrangian multipliers dual ascent time step
K = 3;              % number of modes
